...
export interface IListener {
    ...
    onOfflineMessage(data: any): void; // 拉取到离线消息事件
}
...
export class ImClient {
    ...
    async init(httpUrl: string, appId: number, userId: string, userSign: string, listeners: any, callback: (sdk: ImClient) => void) {
        let self = this;
        ...
        if (success) {
            ...
            conn.onmessage = (evt) => {
                ...
                } else if (typeof imClient.listeners.onTestMessage === 'function') {
    ...
    buildMessagePack(command: number, messagePack: any) {
    ...
    private async errorHandler(error: Error, req: LoginPack) {
        ...
        for (let index = 0; index < 10; index++) {
            ...
            try {
                ...
                let { success, err } = await limLogin(this.url, req, this);
                if (success) {
    ...
    getSingleUserInfo(uid: string): Promise<any> {
        return new Promise((resolve, _) => {
            let api = new HttpApi(this.httpUrl);
    ...
    async syncGetUserInfo(userId: string[]) {
        let api = new HttpApi(this.httpUrl);
    ...
    // 2、心跳
    private heartbeatLoop(conn: w3cwebsocket) {
        let start = Date.now()
        let loop = () => {
    ...
    //构建单聊消息对象
    createP2PTextMessage(to: string, text: string) {
    ...
    sendP2PMessage(pack: MessagePack) {
    ...
    private async loadOfflineMessage() {
        ...
        let api = new HttpApi(this.httpUrl);
        let resp = await api.call("/message/syncOfflineMessage", this.getRequestParams(), {
            clientType: this.clientType,
            appId: this.appId,
